...
# Updated helper to yield actual standalone entity instances based on mock payload dicts
async def async_payload_gen_wrapper(payload_list: Optional[List[Dict]]):
    """Wraps a list of payload dicts in an async generator yielding standalone entity instances."""
    if payload_list is None:
        payload_list = []
    for payload in payload_list:
        # Ensure parent_id is always string if present
        parent_id_keys = ["chunk_of", "defined_in_file", "used_in_file"]
        str_parent_id = None
        for key in parent_id_keys:
            if key in payload:
                str_parent_id = str(payload[key])
                break

        # Instantiate the correct standalone BaseModel subclass based on type
        type_val = payload.get("type")
        dp = None # Initialize dp
        if type_val == "TextChunk":
            ...
            print(f"Warning: Mock payload creator encountered unknown type: {type_val}")
            continue # Use continue to skip in async generator
...
async def test_process_repository_basic_flow(mock_parser_map_dict, mock_discover, tmp_path: Path):
    ...
    # This side effect *is* the async generator
    async def parse_side_effect(file_path, file_id):
        ...
        async for dp in async_payload_gen_wrapper(mock_payloads_for_file):
            yield dp
    ...
    assert isinstance(repo_dp, Repository), f"First item should be Repository instance, got {type(repo_dp)}"
    ...
    assert repo_dp.path == str(repo_path.absolute()), "Repository path mismatch" # Check direct attribute
    ...
    for i, file_dp in enumerate(yielded_files):
        ...
        assert isinstance(file_dp, SourceFile), f"Item {i+1} should be SourceFile, got {type(file_dp)}"
        assert file_dp.type == "SourceFile", f"File {i} type mismatch"
        assert file_dp.id == expected_file_id, f"File {i} ID mismatch"
        ...
        assert file_dp.part_of_repository == expected_outcomes["repo_id"], f"File {i} repo link mismatch"
    ...
    print(f"Expected parser outputs grouped: {expected_parser_outputs_grouped.keys()}")
    yielded_parser_results_grouped = {}
    for dp in parser_results:
        # Determine parent ID based on entity type
        parent_id = None
        # Check types against the new standalone models
        if isinstance(dp, (CodeEntity, TextChunk, Dependency)):
        ...
        parent_id_str = str(parent_id) # Parent ID is already a string field
    ...
    for abs_p, rel_p, f_type in mock_discovery_data:
        if f_type in mock_parser_map_dict: # Only expect calls for supported types
            file_id = expected_outcomes["file_ids"][rel_p] # Get string file ID
            ...
            # Pass the string ID to the mock assertion
...
async def test_process_repository_parser_error_handling(mock_logger, mock_parser_map_dict, mock_discover, tmp_path: Path):
    ...
    # Make the side effect an async generator
    # Make the side effect an async generator
    async def error_parse_side_effect(file_path, file_id):
        ...
        if "ok.py" in file_path:
            ...
            async for dp in async_payload_gen_wrapper(ok_payloads):
                 yield dp
            ...
            await error_raiser() # Directly raise the exception here
        # else: # No need for else; if neither condition matches, the generator simply ends.
        #     # If an empty generator *must* be returned for some reason:
        #     if False: yield # Creates a generator context without yielding
mock_python_parser.parse = AsyncMock(side_effect=error_parse_side_effect)
        ...
        mock_python_parser.parse.assert_any_call(file_path=py_ok_path, file_id=str(file_ok_id)) # Pass string ID
        mock_python_parser.parse.assert_any_call(file_path=py_err_path, file_id=str(file_err_id)) # Pass string ID
