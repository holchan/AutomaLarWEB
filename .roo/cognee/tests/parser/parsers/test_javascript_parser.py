...
async def test_parse_simple_function_file(parser: JavascriptParser, tmp_path: Path, run_parser_and_save_output):
    ...
    assert "function add(a, b)" in func_add.snippet_content
    assert "Adds two numbers" in func_add.snippet_content

    func_multiply = next((f for f in funcs if ":FunctionDefinition:multiply:" in f.id), None)
    assert func_multiply is not None
    assert "const multiply = (a, b) =>" in func_multiply.snippet_content
...
async def test_parse_class_with_imports_file(parser: JavascriptParser, tmp_path: Path, run_parser_and_save_output):
    ...
    cls_file_manager = next((c for c in classes if ":ClassDefinition:FileManager:" in c.id), None)
    assert cls_file_manager is not None
    assert cls_file_manager.type == "ClassDefinition"
...
"""
    ...
    assert "<h1>Hello, {name}!</h1>" in func_my_component.snippet_content
