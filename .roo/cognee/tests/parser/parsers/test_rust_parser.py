...
async def test_parse_simple_mod_file(parser: RustParser, tmp_path: Path, run_parser_and_save_output):
    ...
    assert next((e for e in code_entities if e.type == "ModuleDefinition" and ":utils:" in e.id), None) is not None
    assert next((e for e in code_entities if e.type == "StructDefinition" and ":Point:" in e.id), None) is not None
    assert next((e for e in code_entities if e.type == "EnumDefinition" and ":Status:" in e.id), None) is not None
    assert next((e for e in code_entities if e.type == "TraitDefinition" and ":Summary:" in e.id), None) is not None
    impls = [e for e in code_entities if e.type == "Implementation"]
    assert len(impls) == 2
    assert next((impl for impl in impls if ":Implementation:Point:" in impl.id and "Summary" in impl.snippet_content), None) is not None
    funcs = [e for e in code_entities if e.type == "FunctionDefinition"]
    assert len(funcs) == 5
    ...
    assert next((e for e in code_entities if e.type == "MacroDefinition" and ":create_map:" in e.id), None) is not None
    import_rels = [r for r in rels if r.type == "IMPORTS"]
    assert len(import_rels) == 2
    assert sorted([r.target_id for r in import_rels]) == ["crate::utils::helper", "std::collections::HashMap"]
