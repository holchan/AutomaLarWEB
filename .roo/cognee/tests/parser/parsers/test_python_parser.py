...
async def test_parse_simple_function_file(parser: PythonParser, tmp_path: Path, run_parser_and_save_output):
    ...
    assert len(funcs) == 1
    func_process_data = next((f for f in funcs if ":FunctionDefinition:process_data:" in f.id), None)
    assert func_process_data is not None
    ...
    assert "os" in import_targets and "logging" in import_targets
    file_id_from_chunk = first_chunk.id.rsplit(":",1)[0] if ":" in first_chunk.id else first_chunk.id
    for r in import_rels: assert r.source_id == file_id_from_chunk
...
async def test_parse_class_with_imports_file(parser: PythonParser, tmp_path: Path, run_parser_and_save_output):
    ...
    cls_data_processor = next((c for c in classes if ":ClassDefinition:DataProcessor:" in c.id), None)
    ...
    assert "__init__" in func_names_from_ids and "load_data" in func_names_from_ids and \
           "process" in func_names_from_ids and "main" in func_names_from_ids
    import_rels = [r for r in relationships if r.type == "IMPORTS"]
    assert len(import_rels) >= 3
    import_targets = {r.target_id for r in import_rels}
    assert "asyncio" in import_targets and any(it.startswith("typing") for it in import_targets) and ".utils" in import_targets
...
async def test_parse_file_with_only_comments(parser: PythonParser, tmp_path: Path, run_parser_and_save_output):
    ...
    if content.strip():
        ...
        relationships = [dp for dp in results if isinstance(dp, Relationship)]
        code_entities = [dp for dp in results if isinstance(dp, CodeEntity)]
        assert len(chunks) >= 1 and len(code_entities) == 0
        assert len(relationships) == len(chunks) and all(r.type == "CONTAINS_CHUNK" for r in relationships)
        full_chunked_text = "".join(c.chunk_content for c in chunks)
        assert "# comment1" in full_chunked_text
    else:
        assert len(results) == 0
