...
from .entities import Repository, SourceFile, CodeEntity, Dependency, TextChunk # Import specific entities
...
async def _process_single_file(parser_instance: BaseParser, file_path: str, file_id: str) -> list[BaseModel]:
    """
    ...
    file path and file ID, collects all yielded Pydantic BaseModel objects, and handles
    ...
    Returns:
        A list of Pydantic BaseModel objects yielded by the parser for this file.
...
async def process_repository(repo_path: str, concurrency_limit: int = 50) -> AsyncGenerator[BaseModel, None]:
    ...
    the following steps:
    1. Creates and yields a Repository entity for the given path.
    2. Discovers supported files within the repository using `discover_files`.
    3. For each discovered file, creates and yields a SourceFile entity.
    ...
    8. Yields all Pydantic BaseModel objects (TextChunks, CodeEntities, Dependencies)
    ...
    Yields:
        Pydantic BaseModel objects: First, the Repository node, then SourceFile nodes
        for each discovered file, followed by all BaseModel objects (TextChunks,
